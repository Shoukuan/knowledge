---
title: I2C
type: concept
status: active
tags: ["concept", "I2C", "总线"]
aliases: ["Inter-Integrated Circuit", "I2C bus"]
sources: ["硬件接口/I2C/I2C总线.md", "Linux/驱动/I2C/i2c驱动.md"]
updated_at: 2026-04-28
---
# I2C

I2C 是嵌入式系统里最基础也最常见的低速串行总线之一，当前知识库已经兼顾协议与 Linux 驱动视角。

## 核心问题

- I2C 总线的时序和主从机制如何工作
- 协议理解如何落到 Linux I2C 驱动模型上
- I2C 在系统设计里通常承担哪些设备连接职责

## 相关资料

- [I2C总线](https://blog.csdn.net/guoguo295/article/details/41479353)
- [i2c驱动](../../Linux/驱动/I2C/i2c驱动.md)

## 主题连接

- [[Linux]]
- [[驱动模型]]

## 延伸链接

- [Linux](../entities/linux.md)
- [驱动模型](驱动模型.md)

## 反向链接

<!-- BACKLINKS START -->
- [驱动模型](驱动模型.md)
- [Linux](../entities/linux.md)
- [Wiki Index](../index.md)
- [I2C驱动（资料摘要）](../sources/Linux/驱动/I2C/i2c驱动.md)
- [I2C总线（资料摘要）](../sources/硬件接口/I2C/I2C总线.md)
<!-- BACKLINKS END -->
