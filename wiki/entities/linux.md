---
title: Linux
type: entity
status: active
tags: ["entity", "Linux", "kernel"]
aliases: ["Linux 内核", "Linux kernel"]
sources: ["Linux/Linux内核.md", "Linux/Linux嵌入式系统开发.md", "Linux/linux_kernel_wiki.md"]
updated_at: 2026-04-28
---
# Linux

Linux 是当前知识库中的核心技术主线，覆盖内核结构、启动流程、调试、驱动、内存管理和调度等多个主题。

## 当前关注

- 内核启动与初始化
- 进程调度与中断处理
- 驱动模型与设备树
- 调试与性能分析

## 相关资料

- [Linux内核](../../Linux/Linux内核.md)
- [Linux嵌入式系统开发](../../Linux/Linux嵌入式系统开发.md)
- [linux_kernel_wiki](https://github.com/0voice/linux_kernel_wiki/tree/main/%E6%96%87%E7%AB%A0)

## 主题连接

- [[中断]]
- [[设备树]]
- [[进程调度]]
- [[内存管理]]

## 延伸链接

- [中断](../concepts/中断.md)
- [设备树](../concepts/设备树.md)
- [进程调度](../concepts/进程调度.md)
- [内存管理](../concepts/内存管理.md)

## 反向链接

<!-- BACKLINKS START -->
- [I2C](../concepts/i2c.md)
- [PCIe](../concepts/pcie.md)
- [USB](../concepts/usb.md)
- [中断](../concepts/中断.md)
- [内存管理](../concepts/内存管理.md)
- [启动流程](../concepts/启动流程.md)
- [设备树](../concepts/设备树.md)
- [调试工具](../concepts/调试工具.md)
- [进程调度](../concepts/进程调度.md)
- [驱动模型](../concepts/驱动模型.md)
- [Buildroot](buildroot.md)
- [Trace32](trace32.md)
- [U-Boot](u-boot.md)
- [Wiki Index](../index.md)
- [Linux 内核（资料摘要）](../sources/Linux/Linux内核.md)
- [Linux嵌入式系统开发（资料摘要）](../sources/Linux/Linux嵌入式系统开发.md)
- [linux_kernel_wiki（资料摘要）](../sources/Linux/linux_kernel_wiki.md)
<!-- BACKLINKS END -->
